...
const SESSION_FILE_PATTERN = /^rollout-.*[-_]([0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12})\.jsonl$/i;
...
function findBlockingProcesses(output) {
  return output
    ...
    .map((line) =>
      process.platform === "win32"
        ? parseTaskListCsv(line)[0]?.toLowerCase()
        : line.trim().split(/[\\/]/).pop()?.toLowerCase(),
    )
    .filter(
      (name) =>
...
async function applyMigration(taskId = null) {
  ...
  try {
    ...
    await validateLineageBeforeCommit(paths, manifest);
    manifest.status = "lineage-validated";
    await writeManifest(manifestPath, manifest);
...
async function selfTest() {
  const sample = process.platform === "win32"
    ...
    : ["ChatGPT", "cc-switch", "codex-command-runner-x86_64-pc-windows-msvc", "notepad"].join("\n");
...
async function validateLineageBeforeCommit(paths, manifest) {
  const entries = paths.map((source) => {
    const affected = manifest.files.find((file) => file.source === source);
    return {
      source,
      candidate: affected?.candidate ?? null,
      taskId: taskIdFromSessionPath(source),
    };
  });
  await validateExplicitSessionLineage(entries);
  manifest.lineage = {
    validatedFiles: entries.length,
...
if (mode === "--apply") {
  console.error("修复已停止：当前工具尚不能安全处理跨账号的加密压缩记忆和分页历史。未扫描、备份或修改任务。请勿通过删除校验或密文强行继续。");
  process.exitCode = 2;
}
