...
const SESSION_FILE_PATTERN =
  /^rollout-.*[-_]([0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12})\.jsonl$/i;
...
export async function inspectExplicitSessionFiles(files) {
  ...
  for (const file of files) {
    ...
    const candidateRecords = file.candidate
      ...
      : sourceRecords;
    const recordTaskId = taskIdFromRecords(file.source, sourceRecords);
    const filenameTaskId = basename(file.source).match(SESSION_FILE_PATTERN)?.[1] ?? null;
    const taskId = file.taskId ?? recordTaskId ?? filenameTaskId;
    if (typeof taskId !== "string" || taskId.length === 0) {
      fail(`Cannot determine task ID for ${file.source}`);
    }
    const aliases = [...new Set([taskId, recordTaskId, filenameTaskId].filter(Boolean))];
    const historyBases = [];
    for (const record of candidateRecords) collectHistoryBases(record, historyBases);
    sessions.push({
      taskId,
      strictTaskId: file.taskId ?? null,
      filenameTaskId,
      source: file.source,
...
export async function validateExplicitSessionLineage(files) {
  ...
  for (const session of sessions) {
    if (session.strictTaskId) {
      const existing = byTaskId.get(session.strictTaskId);
      if (existing && existing !== session) fail(`Duplicate task ID in supplied set: ${session.strictTaskId}`);
      byTaskId.set(session.strictTaskId, session);
    }
  ...
  for (const session of sessions) {
    for (const alias of session.aliases) {
      if (byTaskId.has(alias)) continue;
      if (aliasCounts.get(alias) !== 1 && session.filenameTaskId !== alias) continue;
      byTaskId.set(alias, session);
    }
  }
