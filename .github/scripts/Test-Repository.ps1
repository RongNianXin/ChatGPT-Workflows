...
function Test-CommanderDurableWorkflowContract {
    $contracts = @(
        ...
        @{
            ...
            Required = @('为其他任务窗口准备提示词', '唯一模板选择规则', '参数自动核实与人工输入边界', '统一入口的场景与参数核验', '路由回执', '开始自然语言路由前', '当前任务未加载该修订', '自然语言路由别名', '不同 Agent 实例分别执行和独立审查', '普通的同窗口自检', '唯一当前行动与跨窗口冲突收敛', '候选建议永远不可直接执行', '当前阶段行动方（执行者或独立审查者）', '兼任收口方', '操作者直接发给专项窗口的有效指令', '已确认接收 / 已发送待确认 / 尚未送达', '消息到达较晚不代表更新', '某一维度相同不能推出完整测试对象相同', '换窗与归档前连续性触发', '不按固定关键词触发', '至少生成一段可直接发给新窗口的精简续接提示词', '使用者直接点击客户端侧栏归档不会形成模型可观察消息', '不能作为该动作的授权', '需要多个设备访问时', '低信息部署请求与运行身份交付门禁', '规范启动命令及自检输出', 'COMMIT-LEDGER', '保留级别：KEY_NODE', '并存实现决议矩阵', '紧凑文本执行图', '可翻页的本地静态 HTML', '先恢复原任务身份', '恢复提示词本身不得被解释为总指挥任命', '恢复收益门禁', '前台控制授权门禁', '不自动授权 Computer Use', '本地协作画像的一次询问与节点触发', '不再重复询问', '不创建定时任务、后台轮询或独立自动化', '克隆可移植性', '相对路径不是所有场景的强制格式', '下一步提示词和单项确认卡不得重置活跃请求清单', '不能关闭整轮任务', '强制状态回执与空输出兜底', '业务权限不足也返回 `BLOCKED`', '用户手册变更后的全工作流联动核验', '每条指令在执行前都必须完成一次与风险相称的前置检查', '第二次对抗式复审不是默认固定步骤', '操作者明确要求“对抗式审查”“再检查”或同义动作时', '不要求操作者复制新的提示词', '跨任务回执的改进闭环与最终出口硬门禁', '跨项目经验吸收', '可复用规则', '项目特定约定', '未证实建议', '未完成事项', '下一步行动建议')
        },
        @{
        ...
        @{
            ...
            Required = @('已有授权随委派传递', '一次确认，范围内执行', '不能单独证明授权成立', '原始要求的最小必要摘录及可访问来源', '任务名称和原文摘录本身不是身份或授权证明', '不重复执行有副作用的动作', '不得代理', '前台控制的独立授权门禁', '不得为方便观察而升级', '不自动授权控制已有个人浏览器会话', '完成后是否需要第二次对抗式复审')
        },
        @{
    ...
    $contracts += @(
        ...
        @{ Path = 'AGENTS.md'; Required = @('## 唯一日常主工作区', '普通文件修改授权不自动包含这些动作', '日常主工作区是否仍在并对齐默认分支') },
