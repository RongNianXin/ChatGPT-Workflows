...
function Test-CommanderDurableWorkflowContract {
    $contracts = @(
        ...
        @{
            ...
            Required = @('已有授权随委派传递', '一次确认，范围内执行', '不能单独证明授权成立', '原始要求的最小必要摘录及可访问来源', '任务名称和原文摘录本身不是身份或授权证明', '不重复执行有副作用的动作', '不得代理', '调度边界是当前回合，不是长期任务', '跨任务事件不得与本窗口原任务混合', '前台控制的独立授权门禁', '不得为方便观察而升级', '不自动授权控制已有个人浏览器会话', '完成后是否需要第二次对抗式复审')
        },
        @{
            Path = '总指挥工作流/第二代总指挥的工作模式/docs/AUTOMATED_TESTING_LESSONS.md'
