...
function Test-BilingualReadmes {
    ...
    foreach ($relativePath in $chineseReadmes) {
        if ($relativePath -eq '实用小工具/Git仓库状态罗盘/README.md') {
            $pointerChildren = @($trackedFiles | Where-Object { $_ -like '实用小工具/Git仓库状态罗盘/*' })
            if ($pointerChildren.Count -ne 1) {
                $errors.Add("跨仓库指针目录必须只有一个 README：$relativePath")
            }
...
function Test-CommanderDurableWorkflowContract {
    $contracts = @(
        @{
            ...
            Required = @('场景 2D：展示最新修改的运行效果', '【具体目标】', '低信息部署请求与运行身份交付门禁', '效果是否通过，由我实际查看后确认', '场景判断：场景编号', '用自然语言保存计划、进度与成果', '不需要记新的场景号', '保存成功后，AI 必须回读中央入口', '场景五：一个 AI 负责执行，另一个 AI 独立检查', '内部子 Agent、独立任务窗口或混合配对', '普通自检或只读解释不自动建立配对', '专项执行者或独立审查者完成一阶段并把结果交给总指挥后', '已确认接收', '已发送待确认', '尚未送达', '直接发给专项窗口、仍属于当前阶段且不冲突的明确指令照常有效', '只给一个“现在立即做什么”', '当前未完成工作移到新对话继续，或在归档前保存续接材料', '准备归档，请先整理续接材料', '当前 AI 可能收不到这个操作', '等宽文本图', '默认不生成矢量图', '完整单图', '静态模板和构建器', '区分无直观样例和尚未采集', '不伪造效果', '场景 1G：任务意外中断后恢复', '不必使用场景 1G', '场景 1J：工作流更新后通知其他项目总指挥', '不需要填写事件编号、哈希、版本、变化清单或写入范围', '已更新到 <版本>', '不得因为本提示词而改变身份', '恢复收益门禁', '直接重做 / 快速恢复 / 深度恢复 / 必须先核账', '不超过 150 字介绍一次', '不会创建定时任务或后台监控', 'Pe1-(a)：开启或接管操作者画像', 'Pe1-(b)：关闭操作者画像', 'Pe1-(c)：新增外部操作者画像资料', '复制粘贴文本 / 精确文件或附件路径 / 精确直接链接', '当前唯一画像写者', '逐字段明确确认', '测试的数据来源', '版本、输入、步骤和数据来源', '本地发布就绪整理', '异常项只局部暂停', '集中审核一次最终远端载荷')
        },
        @{
            Path = '总指挥工作流/第二代总指挥的工作模式/02-总指挥核心规则.md'
            Required = @('为其他任务窗口准备提示词', '唯一模板选择规则', '参数自动核实与人工输入边界', '统一入口的场景与参数核验', '路由回执', '开始自然语言路由前', '当前任务未加载该修订', '自然语言路由别名', '统一计划、进度与存档语义路由', '写进计划', '中央工作项清单', '已讨论、建议、计划、已实施、已验证', '不同 Agent 实例分别执行和独立审查', '普通的同窗口自检', '唯一当前行动与跨窗口冲突收敛', '候选建议永远不可直接执行', '当前阶段行动方（执行者或独立审查者）', '兼任收口方', '操作者直接发给专项窗口的有效指令', '已确认接收 / 已发送待确认 / 尚未送达', '消息到达较晚不代表更新', '某一维度相同不能推出完整测试对象相同', '换窗与归档前连续性触发', '不按固定关键词触发', '至少生成一段可直接发给新窗口的精简续接提示词', '使用者直接点击客户端侧栏归档不会形成模型可观察消息', '不能作为该动作的授权', '需要多个设备访问时', '低信息部署请求与运行身份交付门禁', '规范启动命令及自检输出', 'COMMIT-LEDGER', '保留级别：KEY_NODE', '并存实现决议矩阵', '紧凑文本执行图', '可翻页的本地静态 HTML', '先恢复原任务身份', '恢复提示词本身不得被解释为总指挥任命', '恢复收益门禁', '前台控制授权门禁', '不自动授权 Computer Use', '本地协作画像的一次询问与节点触发', '不再重复询问', '不创建定时任务、后台轮询或独立自动化', '克隆可移植性', '相对路径不是所有场景的强制格式', '下一步提示词和单项确认卡不得重置活跃请求清单', '不能关闭整轮任务', '请求项—实际动作—证据—状态', '待补答或待确认', '强制状态回执与空输出兜底', '跨项目规则刷新的接纳门禁', '规则刷新manifest.json', '项目问题确实影响后续动作时只另列待办', '当前回合原子完成、回合间跨任务消息优先出队', '独立消息契约', '业务权限不足也返回 `BLOCKED`', '用户手册变更后的全工作流联动核验', '事故证据回归', '常规测试通过不能替代事故证据验收', '每条指令在执行前都必须完成一次与风险相称的前置检查', '第二次对抗式复审不是默认固定步骤', '操作者明确要求“对抗式审查”“再检查”或同义动作时', '不要求操作者复制新的提示词', '跨任务回执的改进闭环与最终出口硬门禁', '跨项目经验吸收', '可复用规则', '项目特定约定', '未证实建议', '未完成事项', '下一步行动建议', '规则根和其唯一本地实例', '默认唯一画像写者', 'Pe1-(a)/(b)/(c)', '逐字段确认', '工作流规则、模板、检查脚本和根 `AGENTS.md` 的写入权属于当前唯一总指挥', '其他任务窗口只提交建议包', '只有 `READY / READY_WITH_RESTRICTIONS` 才生成并交付 `final-*`', 'schema v3', 'LEGACY_UNVERIFIED', 'Prepare-Handoff.mjs', '`event_id` 全链唯一')
        },
        @{
            Path = '总指挥工作流/第二代总指挥的工作模式/03-专项任务卡模板.md'
            Required = @('恢复提示词不改变本任务身份', '执行入口：沿用母任务场景', '恢复提示词不改变本任务身份', '不得执行总指挥接管', '默认无头单 worker', '前台控制须精确授权', '每次请求授权确认前', '给出出口前先回读 `02` 的活跃请求清单', '下一步说明必须让操作者拿来就能做', '无需操作，AI 将继续执行', '标题为“可直接发送”的独立 `text` 代码块', '不能只列“修改 README、重新打包、运行测试”等项目待办', '候选建议标明“候选、不可执行”', '候选不得包装成可直接复制执行的提示词', '候选已由收口方确认接收', '候选已发送待确认', '候选尚未送达', '操作者直接发给本专项窗口的明确指令', '现在只给一个最先动作', '换窗与归档前连续性门禁', '至少一种可续接材料')
        },
        @{
            Path = '总指挥工作流/第二代总指挥的工作模式/04-状态、目标变更与交接规范.md'
            Required = @('耐久 Commit 台账与关键节点', '中央工作项清单与保存语义', '待实施、进行中、阻断、暂缓、待验收', '平行中央计划', '中央工作项清单作为全部非终态事项的主来源', '稳定 ID、状态、优先级、已确认成果、精确断点、下一动作、责任对象、恢复或启动条件、失效条件和证据指针', '只保存聚合事实与指针，不复制详细材料', '规范启动命令及自检输出', 'COMMIT-LEDGER', '保留级别：ROUTINE / KEY_NODE', '通用任务中断恢复与无正式总指挥交接', '不是所有中断任务的必经步骤', '恢复任务”不等于“接管项目', '执行恢复收益门禁', '换窗与归档前连续性门禁', '精简续接提示词的最小字段', '已完成且不得重复', '结果未知', '无待续任务', '使用者未通过消息表达而直接点击客户端归档', '未更新/待复核', '首个主回复末尾介绍一次', 'RECEIVED / BLOCKED / COMPLETED / FAILED', '读取接口不可见与目标没有收到分别记录', '调度任务核验与重建说明', '自动化交接清单', '最后可靠成功截点', '平台任务 ID 仅作为获准运行时的定位线索', 'VERIFY_ONLY', 'REBUILD_CANDIDATE_ONLY', '说明指纹一致只证明说明未漂移', '来源请求事件 + 规范化目标 + 交付类型', '完成且结果已交付', '原创建计划失效', '只返回 `clientThreadId`', 'control_handoff_confidence', 'switch_status', 'runtime_acceptance_status', 'professional_acceptance_status', '机器可读封条', 'expected previous digest', '固定提交顺序', '错误目录中未找到工具', '`draft-*`')
        },
        @{
        ...
        @{
            ...
            Required = @('并存实现决议与实际运行身份', '规范启动命令及自检输出', '代码已包含', '干净环境可复现', '发布内容筛选：用途、规则与精确载荷', '没有分支保护不等于没有项目规则', '被明确排除的内容仍不纳入', '测试依赖不等于产品运行依赖', '源码也可能内嵌像素', '默认只读分析', '最新 worktree 不等于新目录架构获准', '不得静默删测试、削断言或隐藏失败', '最终树与本次将上传的全部提交历史', '最终确认绑定目标、精确文件和历史范围', '不能借公开 Draft 上传', '本地发布就绪整理', '异常只局部阻断', '远端可执行候选', '只在候选稳定后申请远端确认')
        },
        @{
            ...
            Required = @('共享组件、公共接口、运行时选择器', '真实产品回归', '过期的测试契约', '测试装置/环境错误', '受影响测试文件或项目定义的等价测试单元', '可在本地复现的受影响 CI Job 命令', '不能把测试弱化为只断言不存在', '事故证据优先回归', '常规测试通过不能替代事故证据验收', '未能复现')
        },
        @{
        ...
        @{
            ...
            Required = @('已有授权随委派传递', '一次确认，范围内执行', '不能单独证明授权成立', '原始要求的最小必要摘录及可访问来源', '任务名称和原文摘录本身不是身份或授权证明', '普通进度告知只说明实际状态', '不重复执行有副作用的动作', '不得代理', '调度边界是当前回合，不是长期任务', '跨任务事件不得与本窗口原任务混合', '跨项目规则刷新回执', '规则刷新是纯规则加载事件', '规则加载只允许两种最终结果：`PASS` 或 `FAIL`', '项目问题确实影响后续工作时', '前台控制的独立授权门禁', '不得为方便观察而升级', '不自动授权控制已有个人浏览器会话', '完成后是否需要第二次对抗式复审', '不把每次测试、暂存或 Commit 拆成新的确认', '达到该状态后再生成一次最终远端执行卡')
        },
        @{
            Path = '总指挥工作流/第二代总指挥的工作模式/docs/AUTOMATED_TESTING_LESSONS.md'
        ...
        @{
            ...
            Required = @('两次操作者授权', '下一条独立消息', 'active_entry_limit', 'routing_alias_limit', '自然语言路由别名', 'profile_revision', '不得替代本轮授权', '独立应用能力未验证', 'unasked / enabled / paused / disabled / unavailable', '不再展示介绍', '不创建定时任务、后台轮询或独立自动化', '删除公开的 `11-操作者协作画像规范.md` 不是关闭方式', '全新项目的空白上下文', '新项目不再回到 `unasked`', 'Pe1-(c)', '建议写入画像 / 建议写入独立资料档案 / 建议脱敏 / 拒绝写入 / 证据不足', '默认唯一画像写者', '操作者画像资料.local/本人身份资料.local.md', '不得进入跨项目操作者画像', '项目索引、交接快照', '不参与新项目的自动画像接管', 'schema_version: 4')
        },
        @{
    ...
    $contracts += @(
        ...
        @{ Path = ($ruleRoot + '03-专项任务卡模板.md'); Required = @('父子关系和编号沿用原记录', '子项实现或取消不自动算父项验收通过', '任务1暂停且剩1.3', '工作区状态交接（必填）', '结束/暂停回报', '跨任务写入协作事件包', '唯一阻断类型') },
    ...
    $contracts += @(
        @{ Path = ($ruleRoot + '01-操作者操作手册.md'); Required = @('4H 和 4I 的区别', '场景 4I：按需安全汇合双方成果', '场景 1I：交接失败时如何处理', '发送给移交方 AI', '当前恢复置信度为【中/低】', '原因/依据来源为【复制对话窗口原文 / 口述总结 / 其他】', '<a id="场景-2d本地与远端长期分叉后的安全同步"></a>', '不因本提示词自动 Fetch', 'PR 合并后是否又追加提交', '可删除本地引用', '本地分支、远端分支、stash、worktree 分开列项', '本地实际仓库为准') },
        @{ Path = ($ruleRoot + '02-总指挥核心规则.md'); Required = @('发布与同步的共用只读路由', '同 HEAD 不代表工作区相同', '不按文件时间裁定', '远端新提交已包含于本地', '队友未推送成果不可见时标未知', '只有远端有效增量，本地无待汇合增量', '双方有变化但当前暂不汇合', '不预选Merge', '不因通用路由自动Fetch', '回退到操作手册场景 1I', '<a id="场景-2d长期双边分叉的安全汇合"></a>', '不另建第二套发布卡', '原目标仅同步本地时按该目标收口', '唯一日常主工作区', '实际修改目录、分支/HEAD', '`branch --merged` 只证明提交祖先关系', 'PR 后是否追加提交', '删除本地分支、远端分支、stash 和 worktree 是四个独立动作') },
        @{ Path = 'AGENTS.md'; Required = @('## 唯一日常主工作区', '普通文件修改授权不自动包含这些动作', '日常主工作区是否仍在并对齐默认分支', '长期任务是否完成与当前回合是否仍在运行必须分开判断', '回合间空闲点', '独立的最小事件契约', '不得与来信拼成同一个《任务契约》') },
        @{ Path = ($ruleRoot + '09-自动化授权与风险分级.md'); Required = @('不属于普通“修改本地项目”授权', '回归默认分支方式和保留条件', '普通“清理旧分支”只授权生成候选清单', '跨任务成果、授权与写入租约分流', '没有租约”不得伪装成“缺少用户授权') },
        @{ Path = ($ruleRoot + '04-状态、目标变更与交接规范.md'); Required = @('写入租约、成果运输与失效恢复', 'idle` 不等于失效', 'archived/notFound` 也不等于文件或成果已删除') },
        @{ Path = ($ruleRoot + '10-自动状态索引规范.md'); Required = @('不复制第二套租约库', '当前跨任务写入协作事件') },
        @{ Path = ($ruleRoot + 'docs/PR_SUBMISSION_AND_REVIEW_STANDARD.md'); Required = @('合并后的分支收口必须绑定 PR 当时的最终 Head', '分支删除、stash 删除、worktree 移除和 PR 关闭分别授权') }
    )
...
function Test-CommanderBoundedExecutionCases {
    ...
    $guards = @(
        ...
        @{ File='02-总指挥核心规则.md'; Required=@('最后一个身份完整、状态明确的可靠候选', '候选、效果未验收', '身份门禁通过只证明部署对象一致', '多个算法族或组件分别维护版本身份', '只阻断受影响候选的共享部署'); Forbidden=@('最后一次修改的算法作为默认') },
        @{ File='04-状态、目标变更与交接规范.md'; Required=@('核心状态必需', '不为无 GitHub 的本地工作包生成空远端账', '自然语言成果存档', '内容身份与适用范围未变化时复用已有存档', '联合恢复包拥有独立的包身份', '复制内容', '仅记录指纹/取得方式', '不得保存其内容哈希'); Forbidden=@('最小状态记录必须包含：') },
...
function Test-ArchiveRepairLauncher {
    $caseRoot = Join-Path $repoRoot '故障排查与解决经验/01-对话找不到或归档出错/TRB-001-归档时提示文件找不到'
...
function Test-TroubleshootingKnowledgeBase {
    ...
    $cases = @(
        @{ Id = 'TRB-001'; Path = '01-对话找不到或归档出错/TRB-001-归档时提示文件找不到/TRB-001-归档时提示文件找不到.md'; Status = '已解决' },
        @{ Id = 'TRB-002'; Path = '01-对话找不到或归档出错/TRB-002-打开对话时提示找不到/TRB-002-打开对话时提示找不到-恢复方法.md'; Status = '部分解决' },
        @{ Id = 'TRB-003'; Path = '02-登录、账号和模型问题/TRB-003-换账号后原对话不见了/TRB-003-换账号后原对话不见了-历史列表分裂.md'; Status = '部分解决' },
        @{ Id = 'TRB-004'; Path = '02-登录、账号和模型问题/TRB-004-换账号后旧对话打不开/TRB-004-换账号后旧对话打不开-恢复方法.md'; Status = '部分解决' },
        @{ Id = 'TRB-005'; Path = '02-登录、账号和模型问题/TRB-005-转移旧对话后报错/TRB-005-转移旧对话后报错-暂不处理.md'; Status = '未解决' },
        @{ Id = 'TRB-006'; Path = '03-AI任务发消息没回应/TRB-006-发消息给AI任务却没回应-排查与预防.md'; Status = '未解决' },
        @{ Id = 'TRB-007'; Path = '04-网络断开或服务报错/TRB-007-任务太长或网络报错/TRB-007-任务太长或网络报错-总说明.md'; Status = '部分解决' },
        @{ Id = 'TRB-008'; Path = '02-登录、账号和模型问题/TRB-008-官方账号容量不足或总断开/SYS-TRB-008-官方账号容量不足或总断开.md'; Status = '未解决' },
        @{ Id = 'TRB-009'; Path = '02-登录、账号和模型问题/TRB-009-打开项目时找不到模型/SYS-TRB-009-打开项目时找不到模型.md'; Status = '已解决' },
        @{ Id = 'TRB-010'; Path = '05-任务没做完就结束/TRB-010-任务没做完就结束/SYS-TRB-010-任务没做完就结束.md'; Status = '未解决' }
    )
    ...
    $migration = Join-Path $root '02-登录、账号和模型问题/TRB-005-转移旧对话后报错/旧对话转移工具研究-只能用假数据测试/install_bulk_codex_migration.mjs'
