...
function Test-CommanderDurableWorkflowContract {
    $contracts = @(
        ...
        @{
            ...
            Required = @('为其他任务窗口准备提示词', '唯一模板选择规则', '参数自动核实与人工输入边界', '统一入口的场景与参数核验', '路由回执', '开始自然语言路由前', '当前任务未加载该修订', '自然语言路由别名', '统一计划、进度与存档语义路由', '写进计划', '中央工作项清单', '已讨论、建议、计划、已实施、已验证', '不同 Agent 实例分别执行和独立审查', '普通的同窗口自检', '唯一当前行动与跨窗口冲突收敛', '候选建议永远不可直接执行', '当前阶段行动方（执行者或独立审查者）', '兼任收口方', '操作者直接发给专项窗口的有效指令', '已确认接收 / 已发送待确认 / 尚未送达', '消息到达较晚不代表更新', '某一维度相同不能推出完整测试对象相同', '换窗与归档前连续性触发', '不按固定关键词触发', '至少生成一段可直接发给新窗口的精简续接提示词', '使用者直接点击客户端侧栏归档不会形成模型可观察消息', '不能作为该动作的授权', '需要多个设备访问时', '低信息部署请求与运行身份交付门禁', '规范启动命令及自检输出', 'COMMIT-LEDGER', '保留级别：KEY_NODE', '并存实现决议矩阵', '紧凑文本执行图', '可翻页的本地静态 HTML', '先恢复原任务身份', '恢复提示词本身不得被解释为总指挥任命', '恢复收益门禁', '前台控制授权门禁', '不自动授权 Computer Use', '本地协作画像的一次询问与节点触发', '不再重复询问', '不创建定时任务、后台轮询或独立自动化', '克隆可移植性', '相对路径不是所有场景的强制格式', '下一步提示词和单项确认卡不得重置活跃请求清单', '不能关闭整轮任务', '强制状态回执与空输出兜底', '跨项目规则刷新的接纳门禁', 'RULE_REFRESH_ID', '版本、大小、修改时间和 SHA-256 只证明内容身份', '当前回合原子完成、回合间跨任务消息优先出队', '独立消息契约', '业务权限不足也返回 `BLOCKED`', '用户手册变更后的全工作流联动核验', '每条指令在执行前都必须完成一次与风险相称的前置检查', '第二次对抗式复审不是默认固定步骤', '操作者明确要求“对抗式审查”“再检查”或同义动作时', '不要求操作者复制新的提示词', '跨任务回执的改进闭环与最终出口硬门禁', '跨项目经验吸收', '可复用规则', '项目特定约定', '未证实建议', '未完成事项', '下一步行动建议', '规则根和其唯一本地实例', '默认唯一画像写者', 'Pe1-(a)/(b)/(c)', '逐字段确认', '工作流规则、模板、检查脚本和根 `AGENTS.md` 的写入权属于当前唯一总指挥', '其他任务窗口只提交建议包')
        },
        @{
        ...
        @{
            ...
            Required = @('COMMIT-LEDGER', '中央工作项清单的规范化相对路径', '只写专项或会议材料而未同步中央入口不得登记为保存完成', '人工核验运行身份清单', '规范启动命令及自检输出', '并存实现决议矩阵', '节点覆盖状态', 'TASK-RESUME', 'CONTINUITY-PACK', '触发类型（总指挥交接 / 主线分支 / 归档）', '送达状态', '恢复提示词不能把普通或专项任务升级为总指挥', '恢复收益门禁', 'unasked / enabled / paused / disabled / unavailable', 'not-shown / shown / answered / ignored', 'profile_revision', '不发送画像正文', '不创建定时任务或后台轮询', '重新绑定到当前仓库根目录', '活跃请求清单中每项的来源', '单项卡完成后不得据此删除未覆盖项', '自动化交接清单指针', '调度重建说明指针', 'VERIFY_ONLY / REBUILD_CANDIDATE_ONLY / BLOCKED / EXPIRED', 'RULE-REFRESH', '规则刷新按事件触发', '已完成接纳 / WARN / FAIL / 不可达', '项目化复述缺失', '不能直接控制模型注意力', '未实际回读、输出被截断、路径不可访问', '测试数据来源')
        },
        @{
        ...
        @{
            ...
            Required = @('共享组件、公共接口、运行时选择器', '真实产品回归', '过期的测试契约', '测试装置/环境错误', '受影响测试文件或项目定义的等价测试单元', '可在本地复现的受影响 CI Job 命令', '不能把测试弱化为只断言不存在')
        },
        @{
